...
uniform float f = 0.05;           // some coupling parameter
uniform float k = 0.0675;           // another coupling parameter
...
void main()
{
	...
	float du	= ru * sum.g - uvv + F * (1.0 - u);	
	//float dv	= rv * sum.r - uvv + K * (1.0 - v);	
		
	float dv	= rv * sum.r + uvv - (F + K) * v;		

	u += du*0.08 * iAnimationTime;
	v += dv*0.08 * iAnimationTime;
	oColor = vec4( clamp( u, 0.0, 1.0 ), 1.0 - u/v, clamp( v, 0.0, 1.0 ) ,0.9);
}
