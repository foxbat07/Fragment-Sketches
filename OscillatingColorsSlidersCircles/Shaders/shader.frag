...
uniform float numberOfCircles; //slider: 0.1,3.0,1.0
uniform float innerGap; //slider: 0.01,0.2,0.1
...
uniform float redS; //slider: 0.0,1.0,0.4
uniform float greenS; //slider: 0.0,1.0,0.0
uniform float blueS; //slider: 0.0,1.0,0.4
...
void main(void)
{
	...
	for ( float i = 0.1 ; i < numberOfCircles ; i+= innerGap )
		{
		 color += vec3 ( 
		  redS * shapeBorder( st,i + innerRadius, lineWidth + lineWidth2 *( i ) ) , 
		 greenS * 0.1 , 
		 blueS * shapeBorder( st,i + innerRadius, lineWidth + i * lineWidth2 )  );
